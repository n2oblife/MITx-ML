...
def hinge_loss_single(feature_vector, label, theta, theta_0):
    ...
    Returns:
        ...
        parameters.
    """
    z = ylabel*(np.dot(feature_vector,theta) + theta_0 )
    return max(0,1-z)
...
def perceptron(features, labels, T):
    ...
    theta0 = 0
    for _ in range(T):
        order = get_order(features.shape[0])
        for i in order:
...
def average_perceptron(features, labels, T):
	...
	for _ in range(T):
		...
		for i in order:
			...
			theta_sum += theta
			theta0_sum += theta0
	num_iterations = features.shape[0] * T
	theta_avg = theta_sum / num_iterations
	theta0_avg = theta0_sum / num_iterations
	return theta_avg, theta0_avg
...
def pegasos_single_step_update(
    ...
    Returns:
        ...
        completed.
    """
    if label*(np.dot(theta, feature)+theta0) <= 1 :
        theta = (1-eta*L)*theta + eta*label*feature
        theta0 += eta*label
    else :
        theta = (1-eta*L)*theta
    return theta, theta0
...
def pegasos(features, labels, T, L):
    ...
    for _ in range(T):
        ...
        for i in order:
            t += 1
            eta = 1 / np.sqrt(t)
            theta, theta0 = pegasos_single_step_update(features[i], labels[i], L, eta, theta, theta0)
    return theta, theta0
